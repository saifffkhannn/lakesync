...
const IngestionUI = ({
  ...
  const [selection, setSelection] = useState<any>({
    ...
    loadType: loadTypeProp || (devConfig as any).loadType || '',
    srcSchema: '',
    tgtSchema: ''
  });

  // Keep parent loadType in sync whenever selection.loadType changes
  useEffect(() => {
    if (selection.loadType && selection.loadType !== loadTypeProp) {
      setLoadType(selection.loadType);
    }
  }, [selection.loadType]);
  ...
  // FULL=3 steps (no Mapping), INCREMENTAL=4 steps; Ingestion is always the last step
  const ingestionStep = selection.loadType === 'FULL' ? 3 : 4;
    ...
    <div className="min-h-screen bg-slate-50/70">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {step === 1 && (
          <CredentialsStep
            onBack={onBack}
            onABAPContinue={onABAPContinue}
            onMDMContinue={onMDMContinue}
            selection={selection}
        ...
        {/* ══ STEP: Ingestion (step 3 for FULL, step 4 for INCREMENTAL) ══ */}
        {step === ingestionStep && (
          <ExecutionStep
