...
from src.data_extractor import extract_source_data
from src.aws_s3_client import upload_to_s3, move_to_archive_aws
from src.azure_blob_client import upload_to_azure, move_to_archive_azure
from src.gcp_storage_client import upload_to_gcp, move_to_archive_gcp
...
from src.pipeline_state_manager import update_table_status, update_table_metrics
from src.pipeline_state_manager import append_log
from src.metadata_parser import _parse_json_or_csv
...
def full_load(source: str, cloud: str, target: str) -> None:
    ...
    try:
        ...
        for _, row in metadata.iterrows():
            ...
            # -------------------------------------------------------
            try:
                database = row.get("DATABASE") or row.get("source_database") or row.get("src_db")
                schema = row.get("SCHEMA") or row.get("source_schema") or row.get("src_schema")
                table = row.get("TABLE") or row.get("source_table") or row.get("src_table")
                pk_col = row.get("PRIMARY_KEY") or row.get("Primary_key_column") or row.get("primary_keys")
                if not database or not schema or not table or not pk_col:
                    raise KeyError(f"Required fields missing from row keys: {list(row.keys())}")
                primary_keys = _parse_json_or_csv(pk_col)
            except Exception as e:
            ...
            # DB Names
            # -------------------------------
            raw_database = f"{database}_stg"
            tgt_database = database  # Target DB uses exact name as provided (no suffix)
            ...
            try:
                ...
                # -------------------------------------------------------
                # Read all migration reports for checkpointing
                df_logs = pd.DataFrame()
                log_dir = os.path.join(base_dir, "logs")
                if os.path.exists(log_dir):
                    log_files = [os.path.join(log_dir, f) for f in os.listdir(log_dir) if f.startswith("migration_report_") and f.endswith(".csv")]
                    if log_files:
                        try:
                            df_list = [pd.read_csv(f) for f in log_files]
                            df_logs = pd.concat(df_list, ignore_index=True)
                        except Exception as e:
                            logger.warning(f"Failed to read historical log files: {e}")
 
                last_status = None
 
                if not df_logs.empty:
                    db_col = "database_name" if "database_name" in df_logs.columns else "source_database"
                    schema_col = "schema_name" if "schema_name" in df_logs.columns else "source_schema"
                    table_col = "table_name" if "table_name" in df_logs.columns else "source_table"
 
                    df_table = df_logs[
                                (df_logs["source_system"] == source) &
                                (df_logs["cloud"] == cloud) &
                                (df_logs["target_system"] == target) &
                                (df_logs[db_col] == database) &
                                (df_logs[schema_col] == schema) &
                                (df_logs[table_col] == table)
                            ]
        ...
        try:
            ...
            logger.error(f"Failed to save pipeline logs: {str(save_err)}")
        if source_conn:
            source_conn.close()
