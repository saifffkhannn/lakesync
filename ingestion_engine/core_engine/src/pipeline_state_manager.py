...
def _get_or_create_table_locked(table: str) -> Dict[str, Any]:
    existing = next(
        (item for item in pipeline_state["table_status"] if item["table"].lower() == table.lower()),
        None,
    )
...
def _build_steps(status: str) -> Dict[str, str]:
    ...
    if normalized in {"completed", "success", "skipped", "skipped_no_data"}:
...
def update_table_metrics(
    ...
    with _lock:
        ...
        if inserted_rows is not None:
            existing["inserted_rows"] = inserted_rows

        if updated_rows is not None:
            existing["updated_rows"] = updated_rows
...
def _recalculate_progress_locked() -> None:
    ...
    calculated_progress = int((completed_steps / total_steps) * 100)
    pipeline_state["progress"] = max(pipeline_state.get("progress", 0), calculated_progress)
