...
from src.data_extractor import extract_sapsqlserver_data
...
from src.pipeline_state_manager import update_table_status
...
def should_skip_step(last_status, step_name):
    return (
        ...
        str(last_status.get(step_name)) in ["SUCCESS", "SKIPPED"]
    )
...
def full_load(source: str, cloud: str, target: str) -> None:
    ...
    try:
        ...
        Audit_Columns = {k: v.lower() for k, v in Audit_Columns.items()}
        ...
        for _, row in metadata.iterrows():
            ...
            try:
                ...
                if src_count == 0:
                    logger.warning(f"No data found in source for {table}. Skipping pipeline steps.")
                    ...
                    update_table_status(ui_table_name, "skipped_no_data")
                    ...
                    if should_skip_step(last_status, "extraction_status"):
                        ...
                        file_path = last_status["parquet_file_path"]

                        if not file_path:
                            logger.info("Missing parquet_file_path in previous logs")
                            raise Exception("Missing parquet_file_path in previous logs")
                        ...
                        try:
                            ...
                            file_path = extract_sapsqlserver_data(
                                source_conn,
                                local_folder_path,
                                database,
                                schema,
                    ...
                    try:
                        ...
                        if should_skip_step(last_status, "upload_status"):
                            ...
                            cloud_path = last_status["cloud_path"]
                            table_log["upload_status"] = "SKIPPED"
